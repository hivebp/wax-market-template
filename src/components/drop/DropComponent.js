...
const DropComponent = (props) => {
    ...
    const collection =
        collectionData && collectionData.filter((collection) => collection.collection_name === drop.collectionName)[0]

    const templates =
        templateData &&
        templateData.filter((template) =>
            drop.assetsToMint
                .map((asset) => asset.template_id && asset.template_id.toString())
                .includes(template.template_id.toString()),
        )
    ...
    return (
        <Page id="AssetPage">
                    ...
                    templates.map((template) => (
                        <div className="flex flex-col items-center md:justify-center md:flex-row h-auto px-10">
                            <div className="w-full md:w-2/5">
                                <DropImage template={template} />
                            </div>
                            <div className="w-full md:w-3/5 md:px-10">
                                <DropDetails drop={drop} template={template} collection={collection} />
                            </div>
                        </div>
                    ))
                ) : (
            ...
            <div className="relative">
                ...
                >
                    Sold: {drop.currentClaimed}
                    {drop.maxClaimable ? ` / ${drop.maxClaimable}` : ''}
                </p>
            </div>
            {bought ? (
