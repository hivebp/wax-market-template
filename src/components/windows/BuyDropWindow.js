...
function BuyDropPopup(props) {
    ...
    const validateCollection = async (collection_name, amount, comparison_operator) => {
        const assets = await getAssets({ collections: [collection_name], limit: amount + 1, user: userName })
    ...
    const getValidation = async () => {
        ...
        if (proofOwn && proofOwn.group && proofOwn.group.filters) {
            ...
            if (
                (logical_operator === 0 && foundCounter === proofOwn.group.filters.length) ||
    ...
    useEffect(() => {
        if (listingPrice.includes(' WAX')) {
            ...
            getDelphiMedian().then((res) =>
                parseUSDListingPrice(res, amount, parseFloat(listingPrice.replace(' USD', ''))),
            )
        }
    ...
    return (
        ...
        >
            ...
            <div className={cn('relative m-auto h-20 lg:h-8', 'flex justify-evenly flex-wrap lg:justify-end')}>
                <WindowButton text="Cancel" onClick={cancel} className="text-neutral bg-paper border-neutral" />
                <WindowButton
                    text={free ? 'Claim' : 'Purchase'}
                    onClick={purchase}
                    disabled={isLoadingValidation || (drop.authRequired && !fullfilled) ? 'disabled' : ''}
                />
            </div>
            {isLoading ? (
