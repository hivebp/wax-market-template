    ...
    useEffect(() => {
        const initListings = async (page, collection) => {
            if (state.collections)
                getListings({
                    collections: state.collections.filter((item) =>
        ...
        if (type === 'listings' && initialized) initListings(1, collection)
        if (type === 'assets' && initialized) initAssets(1, collection)
        if (type === 'sales' && initialized) initSales(1, collection)
                ...
                >
                    ...
                    {assets && assets['success']
                        ? assets['data'].map((asset, index) => (
                              <AssetCard {...props} index={index} assets={[asset]} key={index} />
                          ))
                        : ''}
                    {sales && sales['success']
                        ? sales['data'].map((sale, index) => (
                              <AssetCard {...props} index={index} sale={sale} assets={sale.assets} key={index} />
                          ))
                        : ''}
                </div>
            )}
